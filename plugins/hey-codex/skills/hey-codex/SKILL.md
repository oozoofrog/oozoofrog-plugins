...
`codex-run.sh` 래퍼로 실행하고 `run_in_background: true`로 호출합니다.
래퍼가 PID 추적, 완료 대기, `process-output.sh` 후처리를 자동 수행합니다.
출력 첫 줄의 `SESSION=<TOKEN>`을 기억하여 상태 확인에 사용합니다.

**read/suggest 모드:**
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/codex-run.sh" exec "$(pwd)" "프롬프트"
```
Bash tool: `run_in_background: true` (타임아웃 없음)
...
**write 모드 (git 저장소):**
```bash
# Step 1: git status 확인 (일반 Bash 호출)
git status --short

# Step 2: codex 실행 (백그라운드)
bash "${CLAUDE_PLUGIN_ROOT}/scripts/codex-run.sh" exec-full-auto "$(pwd)" "프롬프트"
```
Step 2 Bash tool: `run_in_background: true` (타임아웃 없음)

**write 모드 (non-git):**
```bash
# Step 1: pre-snapshot (일반 Bash 호출)
...
완료 알림 수신 후:
```bash
# Step 3: post-snapshot (일반 Bash 호출)
...
| 프로세스 실패 (exit code ≠ 0) | `codex-status.sh`로 상태 확인 후 stderr 표시 + 재시도 여부 결정 |
| 장시간 실행 | `codex-status.sh <SESSION_TOKEN>`으로 진행 상태 확인 가능 |
