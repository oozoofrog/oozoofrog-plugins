...
**키워드 충돌 시 우선순위**: 두 모드의 키워드가 동시에 등장하면, 사용자의 **주요 의도**를 기준으로 선택합니다. "에러 있는지 검토해줘" → review (검토가 주요 동사), "리뷰 결과 에러 수정해줘" → troubleshoot (수정이 주요 동사). 판단이 어려우면 먼저 등장한 동사의 모드를 선택합니다.
