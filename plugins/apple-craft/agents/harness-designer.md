...
{HARNESS_DIR}/features.json에서 `category: "ui"` 기능을 식별하고, 화면이 필요한 기능을 결정합니다.
...
4. **`{HARNESS_DIR}/design-spec.md` 생성**:
...
### Step 6: {HARNESS_DIR}/features.json 업데이트
...
- **사용자 질문 없음**: Planner가 수집한 {HARNESS_DIR}/harness-spec.md의 "사용자 맥락" 활용.
- **Pencil MCP 도구명**: 환경에 따라 접두사가 다를 수 있음. 동적으로 탐지.
- 한국어로 {HARNESS_DIR}/design-spec.md를 작성하되, 토큰명/코드는 원문 유지.
