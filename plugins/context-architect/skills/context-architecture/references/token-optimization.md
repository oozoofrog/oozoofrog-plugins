...
마크다운은 구조적 경계가 모호하여, 대규모 데이터를 포함할 때 LLM이 데이터를 지침으로 오인하는 **지침 누출(Instruction Leakage)** 현상이 발생한다. XML 태그는 명확한 경계를 제공하여 이행 정밀도를 15% 향상시킨다.
...
1. **CLAUDE.md를 시스템 프롬프트 직후에 배치** — 최대 캐시 재사용
2. **CONTEXT.md는 관련 작업 시에만 로드** — 불필요한 캐시 무효화 방지
...
- [ ] 상세 API 문서가 CONTEXT.md로 분리되었는가?
- [ ] 린터로 자동화 가능한 스타일 규칙이 제거되었는가?

### CONTEXT.md 감사

- [ ] Why(의도)에 집중하고 있는가? (What은 코드가 설명)
- [ ] 상위/하위 CONTEXT.md 링크가 유효한가?
- [ ] 표준 라이브러리 설명 같은 노이즈가 없는가?

### 전체 계층 감사

- [ ] 고립된(Orphaned) CONTEXT.md가 없는가?
