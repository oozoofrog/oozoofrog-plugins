...
`$REF/designs/*.md`와 `$REF/designs/manifest.json`은 npm `getdesign@0.6.8` tarball의 바이트 수준 스냅샷이다(현재 69개 브랜드). 상류가 갱신됐는지 확인하려면:
...
이 스크립트는 npm 레지스트리에서 최신 `getdesign` tarball의 `manifest.json`을 내려받아 로컬 `$REF/designs/manifest.json`의 sha256 해시·브랜드 집합과 비교하고, 신규·삭제·본문 변경된 브랜드를 출력한다. 차이가 발견되면 `$SKILL/scripts/sync-designs.sh`로 재동기화하고(그 뒤 `$REF/catalog-index.md`의 표를 수동 갱신), 차이가 없으면 exit 0으로 종료한다.
