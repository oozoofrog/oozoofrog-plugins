---
name: yoda
description: "학습과학 기반 코드 리뷰 및 교육 콘텐츠 생성. /yoda review로 Before/After/Why 기반 리뷰, /yoda share로 5개 포맷(md/web/wiki/slides/pptx) 팀 전파. 코드 리뷰, 코드 분석, 코드 설명, 팀 공유, 온보딩, 기술 세미나, 지식 전파 요청 시 활성화"
argument-hint: "[review <대상> | share <입력> --format md|web|wiki|slides|pptx --audience junior|mid|senior]"
model: opus
---
...
| **share** | 공유, 전파, 세미나, 슬라이드, 위키, 문서, 온보딩, 마크다운, pptx | 교육 콘텐츠 생성 (5개 포맷 지원) |
...
`--format` 미지정 시 사용자에게 5개 포맷 선택지를 제시하고 물어본다.
...
| **pptx** | `pptx` 스킬이 있으면 위임, 없으면 Marp 슬라이드 + 발표자 노트 마크다운으로 생성 | slides 원칙 전체 + 양식 원칙 (Mayer) — 발표자 노트에 상세 설명 배치 |
...
/yoda share --from-review --format pptx         # PowerPoint로 변환
...
- share 모드의 pptx 포맷은 `pptx` 스킬이 설치되어 있으면 PowerPoint로 생성하고, 없으면 Marp 슬라이드 + 별도 발표자 노트 마크다운으로 fallback한다.
