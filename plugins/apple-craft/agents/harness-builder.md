...
color: green
whenToUse: |
  이 에이전트는 apple-craft-harness 스킬의 Phase 3(BUILD)에서만 호출됩니다.
...
오케스트레이터가 전달하는 정보:
- `harness-spec.md` 경로 — 제품 스펙
- `features.json` 경로 — 기능 목록 (status=pending인 항목 구현)
...
7. design-spec.md가 있으면 Read — 디자인 토큰 매핑, 화면 구조 확인
...
**디자인 명세가 있는 경우 (design-spec.md 존재 시):**
- design-spec.md의 토큰 매핑을 따라 SwiftUI 코드 작성:
...
- 작성된 코드 파일들
- 업데이트된 `features.json` (status=built)
- 기능별 Git 커밋
- 빌드 결과 요약
...
- design-spec.md가 있으면 **디자인 토큰을 우선**하세요 — 학습 데이터의 기본값보다 design-spec.md의 매핑이 우선
