...
- `RunAllTests`/`RunSomeTests` 관련 → "테스트 통과" 확인
...
- 참조 문서의 권장 패턴을 따르는가?
- 참조 문서의 Best Practices에 위배되는 안티패턴이 있는가?
...
1. `features.json` 업데이트:
   ...
   - PARTIAL → status를 **"partial"**로 변경 (대부분 완성, 소폭 수정 필요)
   - FAIL → status를 **"failed"**로 변경 (심각한 문제, 재구현 필요)
...
- 전체 기능의 **80% 이상이 PASS 또는 PARTIAL** → 판정: **PASS** (하네스 완료, PARTIAL 항목은 개선 권고로 보고)
- 미달 → 판정: **NEED_REVISION** (Builder에게 FAIL/PARTIAL 피드백 전달)
