...
오케스트레이터가 전달하는 정보:
- `harness-spec.md` 경로 — 제품 스펙 (사용자 맥락 포함)
- `features.json` 경로 — 기능 목록
...
1. `harness-spec.md` 읽기 — "사용자 맥락" 섹션에서 디자인 취향 확인
...
features.json에서 `category: "ui"` 기능을 식별하고, 화면이 필요한 기능을 결정합니다.
...
- **사용자 질문 없음**: Planner가 수집한 harness-spec.md의 "사용자 맥락" 활용.
- **Pencil MCP 도구명**: 환경에 따라 접두사가 다를 수 있음. 동적으로 탐지.
- 한국어로 design-spec.md를 작성하되, 토큰명/코드는 원문 유지.
