...
1. 수정이 실제로 파일에 적용되었는가? — 파일을 다시 Read하여 확인
2. 수정이 새로운 문제를 유발하지 않았는가? — 영향 범위 연쇄 검증
3. findings를 삭제하거나 심각도를 낮춘 것은 수정이 아니다
4. **의심스러우면 불통과** — 점수 경계값(6-7)에서는 불통과로 판정
5. **자기칭찬 금지** — "잘 수정되었다"는 판단 전에 반드시 파일을 다시 Read
...
- `verify-round-{N}.md`: 2축 점수, 안티패턴 탐지 결과, 수정 지침
