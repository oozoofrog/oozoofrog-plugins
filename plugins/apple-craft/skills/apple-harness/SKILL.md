...
**HARNESS_BASE / HARNESS_DIR 설정:**

베이스 경로는 **프로젝트 루트의 `.apple-harness/`** 로 고정합니다 (`.codex-research/`, `.claude/` 등과 동일한 hidden-dir 컨벤션).
...
- `{project-root}` = 현재 작업 디렉토리(하네스 호출 시 CWD). `pwd`로 확정 후 절대 경로로 저장합니다.
- 예: 프로젝트 루트가 `/Users/me/app`이고 세션 이름이 `liquid-glass-settings`면 → `HARNESS_DIR = /Users/me/app/.apple-harness/liquid-glass-settings`
- `.apple-harness/`는 로컬 작업 산출물이므로 **`.gitignore`에 `.apple-harness/`를 추가**하는 것을 권장합니다 (세션 시작 시 `.gitignore` 미포함이면 사용자에게 알림).
...
**충돌 회피 규칙:** `{HARNESS_BASE}/{session-name}` 디렉토리가 이미 존재하면서 내부에 `session.json`/`features.json`이 있으면 **재진입 대상**(Step 1으로 진행). 충돌하지만 재진입이 아닌 새 세션을 원하면 `-2`, `-3` 접미사를 붙여 새 디렉토리를 만듭니다.

이후 모든 Phase에서 `{HARNESS_DIR}`을 사용합니다 (항상 **절대 경로**):
...
- `{HARNESS_DIR}/guides/`, `{HARNESS_DIR}/reviews/`, `{HARNESS_DIR}/build-errors/`, `{HARNESS_DIR}/worst-cases.md` (Phase 3 collaborative/user-led 모드에서 생성)

모든 에이전트 호출 시 프롬프트에 `HARNESS_DIR: {절대 경로}` 를 반드시 전달합니다 (상대 경로 금지 — 서브에이전트 CWD가 달라질 수 있음).
