...
이 플러그인은 `ui-verifier` 에이전트를 함께 제공합니다. 목적은 **자동화 실행**과 **자동화 검증**을 분리하는 것입니다.
...
1. **actor 단계** — `run_steps`, `tap`, `type_text`, `launch_app` 등으로 플로우 수행
2. **observer 단계** — `ui-verifier`가 `query_ui`, `analyze_ui`, `screenshot`로 근거 점검
3. **종료 게이트** — 이 skill의 `Stop` agent hook가 마지막 응답 전에 검증 누락을 잡아냄
...
### ui-verifier의 hard gate

- `doctor` 또는 동등한 환경 점검 성공
- 인터랙션 전 UI 상태 근거 존재
- 인터랙션 후 기대 상태 근거 존재
- screenshot/video/UI tree 중 하나 이상 존재
- 실패 시 막힌 단계 + 재시도 전략 보고

> screenshot만 있고 selector 근거가 없으면 기본 판정은 PASS가 아니라 `REFINE`입니다.
